...
class rulesTree(object):
    def __init__(self, rules, rule_tree_root_name='rules', data_tree_root_name='data'):
    ...
    def build_data_tree(self, f):
        return self.__build_data_tree(f, self.rule_tree_root, self.data_tree_root)
        ...
        for i in f:
            ...
            if find==False:
                if len(rule_tree_root.children)==0:
                    ...
                    r, cur_parent_data_node =\
                            self.scan_rule_node(i, cur_rule_node, cur_data_node)
                    if r:
                        find = True
                        cur_rule_node = r
                        p_list = cur_rule_node.propety.keys()
